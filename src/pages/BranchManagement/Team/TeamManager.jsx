...
import { Plus, Search, MoreVertical, Edit2, Key, UserX, Users, Shield, Briefcase, Building, Mail, X, Filter, ChevronDown, ChevronRight, CheckCircle, Clock, MapPin, Phone, ArrowLeft } from 'lucide-react';
...
const TeamManager = () => {
  ...
  useEffect(() => {
    ...
    if (user && userRole === 'BRANCH_MANAGER') {
      const branchName = typeof user.branch === 'object' ? user.branch?.name : user.branch;
      if (branchName) setBranchFilter(branchName);
    }
  }, [user]);
  ...
  const [branchFormData, setBranchFormData] = useState({ id: null, name: '', phone: '', email: '', city: '' });
  ...
  const uniqueBranches = ['ALL', ...new Set([...users.map(u => u.branch), ...branches.map(b => b.name)].filter(Boolean))];

  const groupedUsers = {};
  
  // Initialize empty branches so they show up on the grid
  if (searchTerm === '') {
    branches.forEach(b => {
      if (branchFilter === 'ALL' || branchFilter === b.name) {
        groupedUsers[b.name] = [];
      }
    });
  }
  ...
  filteredUsers.forEach(user => {
    ...
    if (!groupedUsers[branchName]) {
      groupedUsers[branchName] = [];
    }
    groupedUsers[branchName].push(user);
  });

  // Ensure strict branch filtering
  if (branchFilter !== 'ALL') {
    Object.keys(groupedUsers).forEach(key => {
      if (key !== branchFilter) delete groupedUsers[key];
    });
  }
  ...
  const handleToggleStatus = async (userId) => {
    ...
    try {
      ...
      console.error(err);
      setUsers(prev => prev.map(u => u.id === userId ? { ...u, status: userToToggle.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE' } : u));
    }
  };

  const handleEditBranchClick = (branchInfo, branchName) => {
    if (!branchInfo || !branchInfo.id) {
      alert("This is an unassigned or mock branch. Cannot edit.");
      return;
    }
    setBranchFormData({
      id: branchInfo.id,
  ...
  const handleToggleBranchStatus = async (branchInfo, branchName) => {
    if (!branchInfo || !branchInfo.id) {
      alert("This is an unassigned or mock branch. Cannot toggle status.");
      return;
    }
    try {
      const newStatus = branchInfo.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE';
      await api.put(`/branches/${branchInfo.id}`, { status: newStatus });
      fetchBranches();
    } catch (err) {
      console.error(err);
      // MOCK UPDATE IF API FAILS
      setBranches(prev => prev.map(b => b.id === branchInfo.id ? { ...b, status: branchInfo.status === 'ACTIVE' ? 'INACTIVE' : 'ACTIVE' } : b));
    }
  };
  ...
  const handleBranchSubmit = async (e) => {
    e.preventDefault();
    try {
      await api.put(`/branches/${branchFormData.id}`, branchFormData);
      fetchBranches();
      setIsBranchModalOpen(false);
    } catch (err) {
      console.error(err);
      // MOCK UPDATE IF API FAILS
      setBranches(prev => prev.map(b => b.id === branchFormData.id ? { ...b, ...branchFormData } : b));
      setIsBranchModalOpen(false);
    }
  };
  ...
  return (
    <div className="max-w-7xl mx-auto pb-10 animate-in fade-in duration-300">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-6">
        <div>
          <h1 className="text-2xl font-bold text-slate-800 tracking-tight">
            {selectedBranch ? `${selectedBranch} Team` : 'Team & Users'}
          </h1>
          <p className="text-sm text-slate-700 mt-1">
            {selectedBranch ? `Manage staff for ${selectedBranch} branch.` : 'Manage your staff, assign roles, and control access to the CRM.'}
          </p>
        </div>
        <div className="flex items-center gap-3">
          {selectedBranch && (
            <button 
              ...
              className="bg-white border border-slate-200 text-slate-700 hover:bg-slate-50 px-4 py-2.5 rounded-lg text-sm font-semibold flex items-center justify-center gap-2 shadow-sm transition-colors whitespace-nowrap"
            >
      ...
      {/* Filter and Search Bar (Only when no branch selected) */}
      {!selectedBranch && (
        <div className="bg-slate-50/50 p-5 rounded-xl border border-slate-200 shadow-sm mb-6 flex flex-col gap-4 relative z-20 animate-in fade-in zoom-in-95 duration-300">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-x-5 gap-y-4 items-end">
             ...
             {/* Hide Select Branch for Branch Managers */}
             {(!user || user?.role?.toUpperCase() === 'SUPER_ADMIN' || user?.role?.toUpperCase() === 'ADMIN') && (
               <CustomSelect 
                 label="Select Branch"
      ...
      {/* Main Content Area */}
      {!selectedBranch ? (
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6 mb-10 items-start">
          {Object.keys(groupedUsers).length === 0 ? (
            <div className="bg-white rounded-2xl border border-slate-200 p-16 text-center shadow-sm col-span-full">
              ...
              <div className="text-slate-800 font-bold text-xl mb-2">No users found</div>
              <div className="text-slate-500 text-sm">Try adjusting your search query or filters.</div>
            </div>
          ) : (
            Object.keys(groupedUsers).map(branch => {
              const branchInfo = branches.find(b => b.name === branch) || {};
              return (
              <div key={branch} className="bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm hover:shadow-md transition-shadow flex flex-col animate-in fade-in slide-in-from-bottom-4">
                
                {/* Branch Card Header (Clickable) */}
                <div 
                  className="p-5 cursor-pointer bg-white hover:bg-slate-50/50 transition-colors flex flex-col h-full"
                  onClick={() => handleBranchSelect(branch)}
                >
                  <div className="flex justify-between items-start mb-5">
                    <div className="flex items-center gap-3">
                      <div className="w-10 h-10 rounded-lg bg-slate-50 text-slate-700 flex items-center justify-center border border-slate-100 shrink-0">
                        <Building size={20} />
                      </div>
                      <div>
                        <h2 className="text-[15px] font-bold text-slate-800 leading-tight mb-1">{branch}</h2>
                        <div className="flex items-center gap-2">
                          <span className={`text-[10px] font-bold flex items-center gap-1 uppercase tracking-wider px-1.5 py-0.5 rounded ${branchInfo.status === 'INACTIVE' ? 'text-rose-600 bg-rose-50' : 'text-emerald-600 bg-emerald-50'}`}>
                            <span className={`w-1.5 h-1.5 rounded-full ${branchInfo.status === 'INACTIVE' ? 'bg-rose-500' : 'bg-emerald-500'}`}></span> {branchInfo.status || 'ACTIVE'}
                          </span>
                          <span className="text-[11px] font-medium text-slate-500">{groupedUsers[branch].length} Employees</span>
                        </div>
                      </div>
                    </div>
                    <div className="relative inline-block text-left" onClick={(e) => e.stopPropagation()}>
                      <button 
                        onClick={() => setOpenDropdownId(openDropdownId === `branch-${branch}` ? null : `branch-${branch}`)}
                        className="p-1 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded transition-colors" 
                      >
                        <MoreVertical size={16} />
                      </button>
                      {openDropdownId === `branch-${branch}` && (
                        <div className="absolute right-0 top-full mt-2 w-48 bg-white rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.1)] border border-slate-100 z-[100] py-1 animate-in fade-in zoom-in-95 duration-100">
                           <button
                             onClick={() => {
                               handleEditBranchClick(branchInfo, branch);
                               setOpenDropdownId(null);
                             }}
                           ...
                           >
                             <Edit2 size={14} className="text-blue-500" /> Edit Branch
                           </button>
                           <div className="h-px bg-slate-100 my-1 mx-2"></div>
                           <button
                             onClick={() => {
                               handleToggleBranchStatus(branchInfo, branch);
                               setOpenDropdownId(null);
                             }}
                             className={`w-full text-left px-4 py-2 text-xs font-medium flex items-center gap-2 transition-colors ${branchInfo?.status === 'INACTIVE' ? 'text-emerald-600 hover:bg-emerald-50' : 'text-rose-600 hover:bg-rose-50'}`}
                           >
                             <Building size={14} /> {branchInfo?.status === 'INACTIVE' ? 'Activate Branch' : 'Deactivate Branch'}
                           </button>
                        </div>
                      )}
                    </div>
                  ...
                  <div className="flex flex-col gap-2.5 text-xs text-slate-600 font-medium">
                    ...
                    <div className="flex items-center gap-2.5">
                      ...
                      {branchInfo.phone || '+91 9876543210'}
                    </div>
                    <div className="flex items-center gap-2.5 break-all">
                      <Mail size={14} className="text-slate-400" /> 
                      {branchInfo.email || `${branch.toLowerCase().replace(/[^a-z0-9]/g, '')}@gmail.com`}
                    </div>
                  </div>
                </div>
              </div>
        ...
        /* Selected Branch Table View */
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden shadow-sm animate-in fade-in slide-in-from-right-8 duration-300 mb-10 pb-32">
          <div className="overflow-x-auto">
            <table className="w-full text-left border-collapse min-w-[800px]">
              <thead>
                <tr className="border-b border-slate-200 text-slate-500 font-bold text-[11px] uppercase tracking-wider bg-slate-50/80">
              ...
              <tbody className="bg-white">
                {groupedUsers[selectedBranch] ? groupedUsers[selectedBranch].map((u, index) => {
                  const totalUsers = groupedUsers[selectedBranch].length;
                  const isLast = index === totalUsers - 1;
                  const shouldOpenUpwards = totalUsers > 3 && index >= totalUsers - 2;
                  return (
                    <tr key={u.id} className={`align-middle hover:bg-slate-50/50 transition-colors ${!isLast ? 'border-b border-slate-100' : ''}`}>
                      <td className="px-6 py-4">
                        <div className="flex items-center gap-4">
                          ...
                          <div>
                            <div className="font-bold text-slate-800 text-[14px] mb-0.5">{u.name}</div>
                      ...
                      <td className="px-6 py-4 text-slate-600 font-medium text-xs align-middle">
                        {u.joined}
                      </td>
                      <td className="px-6 py-4 text-center align-middle">
                        <div className="relative inline-block text-left">
                          <button 
                            onClick={(e) => {
                              e.stopPropagation();
                              setOpenDropdownId(openDropdownId === `tbl-${u.id}` ? null : `tbl-${u.id}`);
                            }}
                            className="p-1.5 text-slate-400 hover:text-slate-800 hover:bg-slate-100 rounded-lg transition-colors" 
                          >
                            <MoreVertical size={18} />
                          </button>
                          {openDropdownId === `tbl-${u.id}` && (
                            <div className={`absolute right-0 w-48 bg-white rounded-lg shadow-[0_4px_20px_rgba(0,0,0,0.1)] border border-slate-100 z-[100] py-1 animate-in fade-in zoom-in-95 duration-100 ${shouldOpenUpwards ? 'bottom-full mb-2' : 'top-full mt-2'}`}>
                              <button
                                onClick={(e) => {
                                  e.stopPropagation();
                                  handleEditClick(u);
                              ...
                              <button
                                ...
                                className={`w-full text-left px-4 py-2 text-sm font-medium flex items-center gap-2 transition-colors ${u.status === 'ACTIVE' ? 'text-rose-600 hover:bg-rose-50' : 'text-emerald-600 hover:bg-emerald-50'}`}
                              >
                  ...
                  <tr>
                    <td colSpan="5" className="px-6 py-8 text-center text-slate-500 font-medium">No users found for this branch.</td>
                  </tr>
                )}
              </tbody>
            </table>
              ...
              <div className="p-6 overflow-y-auto custom-scrollbar flex-1 space-y-4">
                ...
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">
                    {isEditing ? 'Set New Password (Optional)' : 'Temporary Password'}
                  </label>
                  <input 
      ...
      {isBranchModalOpen && createPortal(
        <div className={`fixed inset-0 z-[100] flex items-center justify-center p-4 bg-slate-900/50 backdrop-blur-sm animate-in fade-in duration-200`}>
          <div className="bg-white rounded-2xl shadow-xl w-full max-w-md flex flex-col overflow-hidden animate-in slide-in-from-bottom-8">
            <div className="px-6 py-4 border-b border-slate-100 flex justify-between items-center bg-slate-50/50">
              <div>
                <h3 className="text-lg font-bold text-slate-800">Edit Branch</h3>
                <p className="text-xs text-slate-700 mt-0.5">Update branch details.</p>
              </div>
              <button 
                onClick={() => setIsBranchModalOpen(false)}
                className="text-slate-800 hover:bg-slate-100 p-1.5 rounded-lg transition-colors"
              >
                <X size={20} />
              </button>
            </div>

            <form onSubmit={handleBranchSubmit} className="flex flex-col">
              <div className="p-6 space-y-4">
                <div className="space-y-1.5">
                  <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Branch Name</label>
                  <input 
                    type="text" required
                    value={branchFormData.name} onChange={(e) => setBranchFormData({...branchFormData, name: e.target.value})}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="text" required
                    value={branchFormData.city} onChange={(e) => setBranchFormData({...branchFormData, city: e.target.value})}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="text"
                    value={branchFormData.phone} onChange={(e) => setBranchFormData({...branchFormData, phone: e.target.value})}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
                  />
                </div>
                <div className="space-y-1.5">
                  ...
                  <input 
                    type="email"
                    value={branchFormData.email} onChange={(e) => setBranchFormData({...branchFormData, email: e.target.value})}
                    className="w-full px-3 py-2 bg-slate-50 border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-colors"
                  />
                </div>
              </div>
              <div className="p-6 pt-2 border-t border-slate-100 bg-slate-50/50 flex justify-end gap-3">
                <button 
                  type="button"
                  onClick={() => setIsBranchModalOpen(false)}
                  className="px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 rounded-lg text-sm font-semibold transition-colors"
